package iskallia.itraders.gui.component;
...
public class GuiScrollbar extends Gui {
	...
	private boolean intersects(int x, int y) {
		...
		return scrollX <= x && x <= scrollX + scrollW && scrollY <= y && y <= scrollY + scrollH;
	}
	...
	public void drawScrollbar() {
		...
		this.drawTexturedModalRect(knobX, knobY, knobU + uOffset, knobV, knobWidth, knobHeight);
		this.drawTexturedModalRect(knobX, knobY - knobUpperH, knobUpperU + uOffset, knobUpperV, knobWidth, knobUpperH);
		this.drawTexturedModalRect(knobX, knobY + knobHeight, knobLowerU + uOffset, knobLowerV, knobWidth, knobLowerH);
	}
